...
ggplot( data = ft) +
    geom_bar(mapping = aes(x = reorder(x = country, X = country, FUN = length))) +
    coord_flip()

#  2. Total de jogadores por equipe.

ggplot( data = ft) +
    geom_bar(mapping = aes(x = reorder(x = team, X = team, FUN = length))) +
    coord_flip()

#  3. Total de gols por equipe.

ggplot( data = ft) +
    geom_bar(mapping = aes(x = team, y = goal), stat="sum" ) +
    coord_flip()

#  4. Total de cartões amarelos por equipe.

ggplot( data = ft) +
    geom_bar(mapping = aes(x = team, y = yel), stat="sum" ) +
    coord_flip()
...
ggplot(data = ft) +
    geom_histogram(mapping = aes(x = cm))
...
ggplot(data = ft) +
    geom_point(mapping = aes(x = cm, y = kg))

# 11. Relação do peso com altura por posição em campo.

ggplot(data = ft) +
    geom_point(mapping = aes(x = cm, y = kg)) +
    facet_wrap( facets = ~pos)
